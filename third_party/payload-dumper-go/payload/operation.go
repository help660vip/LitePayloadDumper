...
func (x *partitionExtractor) readBlob(op *chromeos_update_engine.InstallOperation) ([]byte, error) {
	...
	reader := &contextReader{ctx: x.ctx, r: x.blobSectionReader(op)}
...
func (x *partitionExtractor) runReplace(op *chromeos_update_engine.InstallOperation) error {
	...
	h := sha256.New()
	blob := &contextReader{ctx: x.ctx, r: x.blobSectionReader(op)}
	...
	case chromeos_update_engine.InstallOperation_REPLACE_XZ:
		rd, err = newXZReader(tee)
	case chromeos_update_engine.InstallOperation_ZSTD:
		rd, err = newZstdReader(tee)
	}
	if err != nil {
		return fmt.Errorf("initializing decompressor: %w", err)
	}
	progressWriter := &contextProgressWriter{ctx: x.ctx, w: w, onWrite: x.onBytes}
	n, err := io.Copy(progressWriter, rd)
	rd.Close()
	if err != nil {
...
func (x *partitionExtractor) runZero(op *chromeos_update_engine.InstallOperation) error {
	...
	progressWriter := &contextProgressWriter{ctx: x.ctx, w: w, onWrite: x.onBytes}
	if _, err := io.CopyN(progressWriter, zeroReader{}, expected); err != nil {
...
func (x *partitionExtractor) runSourceCopy(op *chromeos_update_engine.InstallOperation) error {
	...
	h := sha256.New()
	reader := &contextReader{ctx: x.ctx, r: io.TeeReader(r, h)}
	progressWriter := &contextProgressWriter{ctx: x.ctx, w: w, onWrite: x.onBytes}
	n, err := io.Copy(progressWriter, reader)
	if err != nil {
...
func (x *partitionExtractor) runBsdiff(op *chromeos_update_engine.InstallOperation) error {
	...
	if _, err := io.ReadFull(&contextReader{ctx: x.ctx, r: r}, srcData); err != nil {
	...
	dstBytes := int64(totalBlocks(dst) * x.p.blockSize)
	newData, err := bspatch.ApplyContext(x.ctx, srcData, patch, dstBytes)
	if err != nil {
	...
	progressWriter := &contextProgressWriter{ctx: x.ctx, w: w, onWrite: x.onBytes}
	if _, err := progressWriter.Write(newData); err != nil {

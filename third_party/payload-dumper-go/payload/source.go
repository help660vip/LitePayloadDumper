...
func openSourceImage(ctx context.Context, dir string, part *chromeos_update_engine.PartitionUpdate, skipVerify bool, logger *slog.Logger) (*sourceImage, error) {
	...
	if needsFullSourceVerification(part) && len(oldInfo.GetHash()) > 0 {
		...
		h := sha256.New()
		reader := &contextReader{ctx: ctx, r: io.NewSectionReader(f, 0, int64(oldInfo.GetSize()))}

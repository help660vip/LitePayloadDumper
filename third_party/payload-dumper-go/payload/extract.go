...
import (
	...
	"encoding/hex"
	"fmt"
	slog "github.com/ssut/payload-dumper-go/internal/loglite"
	"io"
	"os"
	"path/filepath"
	"runtime"
...
func (p *Payload) extractPartition(ctx context.Context, part *chromeos_update_engine.PartitionUpdate, opts ExtractOptions, logger *slog.Logger) (err error) {
	...
	if partitionNeedsSource(part) {
		...
		src, err = openSourceImage(ctx, opts.SourceDir, part, opts.SkipVerify, logger)
		if err != nil {
			return err
		}
		defer src.Close()
	...
	if src != nil {
		stage = "复制旧镜像"
		report(completed, false, nil)
		if err = copySourceFallthrough(ctx, out, src, part, p.blockSize, addBytes, logger); err != nil {
			return err
		}
	}
	...
	stage = "提取"
	report(completed, false, nil)
	x := &partitionExtractor{p: p, name: name, out: out, src: src, skipVerify: opts.SkipVerify, ctx: ctx, onBytes: addBytes}
	...
	if src != nil {
		stage = "生成校验数据"
		report(completed, false, nil)
		if err = writeHashTree(ctx, out, part, p.blockSize, logger); err != nil {
			return err
		}
	}
	...
	stage = "校验输出"
	report(completed, false, nil)
	if err = p.verifyOutput(ctx, out, part, opts.SkipVerify, logger); err != nil {
		return err
	}
...
func copySourceFallthrough(ctx context.Context, out *os.File, src *sourceImage, part *chromeos_update_engine.PartitionUpdate, blockSize uint64, onBytes func(int64), logger *slog.Logger) error {
	...
	for _, gap := range gaps {
		...
		reader := &contextReader{ctx: ctx, r: io.NewSectionReader(src.f, offset, end-offset)}
		writer := &contextProgressWriter{ctx: ctx, w: io.NewOffsetWriter(out, offset), onWrite: onBytes}
...
func (p *Payload) verifyOutput(ctx context.Context, out *os.File, part *chromeos_update_engine.PartitionUpdate, skipVerify bool, logger *slog.Logger) error {
	newInfo := part.GetNewPartitionInfo()
	if skipVerify || len(newInfo.GetHash()) == 0 {
		return nil
	}
	h := sha256.New()
	reader := &contextReader{ctx: ctx, r: io.NewSectionReader(out, 0, int64(newInfo.GetSize()))}

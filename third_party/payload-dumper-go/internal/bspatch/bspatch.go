...
func Apply(old, patch []byte, maxNewSize int64) ([]byte, error) {
	return ApplyContext(context.Background(), old, patch, maxNewSize)
}

func ApplyContext(ctx context.Context, old, patch []byte, maxNewSize int64) ([]byte, error) {
	...
	ctrl = &cancelReader{ctx: ctx, reader: ctrl}
	diff = &cancelReader{ctx: ctx, reader: diff}
	extra = &cancelReader{ctx: ctx, reader: extra}
	...
	for newpos < newSize {
		...
		for chunkStart := int64(0); chunkStart < diffSize; chunkStart += 64 << 10 {
			if err := ctx.Err(); err != nil {
				return nil, err
			}
			chunkEnd := chunkStart + (64 << 10)
			if chunkEnd > diffSize {
				chunkEnd = diffSize
			}
